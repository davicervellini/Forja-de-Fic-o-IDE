...
class AddChapterDialog(customtkinter.CTkToplevel):
    def __init__(self, parent):
        ...
        customtkinter.CTkLabel(
            self, text="Premissa do capítulo",
            font=customtkinter.CTkFont(size=15, weight="bold"),
            text_color=TEXT,
        ).grid(row=0, column=0, padx=20, pady=(18, 6), sticky="w")

        self.textbox = customtkinter.CTkTextbox(
            self, font=customtkinter.CTkFont(family="Consolas", size=13),
        ...
        customtkinter.CTkButton(
            btn_frame, text="📂 Arquivo", command=self._load_file,
            fg_color=BG_CARD, hover_color=BG_HOVER, width=140,
        ).grid(row=0, column=0, padx=4)

        customtkinter.CTkButton(
            btn_frame, text="Adicionar", command=self._confirm,
            fg_color=ACCENT_GREEN, hover_color="#059669", width=140,
...
class WikiImportDialog(customtkinter.CTkToplevel):
    """Importa um ou vários personagens da Wiki em fila."""
    ...
    def __init__(self, parent, project: StoryProject):
        ...
        customtkinter.CTkLabel(
            self, text="Personagens → Memória Dinâmica",
            font=customtkinter.CTkFont(size=15, weight="bold"), text_color=TEXT,
        ).grid(row=0, column=0, padx=18, pady=(16, 2), sticky="w")

        customtkinter.CTkLabel(
            self, text="Um nome por linha. Processados em ordem.",
            text_color=TEXT_DIM, font=customtkinter.CTkFont(size=11),
        ).grid(row=1, column=0, padx=18, pady=(0, 6), sticky="w")

        self.names_tb = customtkinter.CTkTextbox(
            self, font=customtkinter.CTkFont(size=13),
            fg_color=BG_DARK, border_color=BORDER, border_width=1, height=140,
        )
        ...
        customtkinter.CTkLabel(frame, text="Franquia", text_color=TEXT_DIM).grid(
            row=0, column=0, padx=(0, 8), sticky="e"
        )
        from pipeline.wiki_fetcher import WIKI_DOMAINS
        self.franchise_cb = customtkinter.CTkComboBox(
            frame, values=list(WIKI_DOMAINS.keys()), fg_color=BG_CARD, border_color=BORDER,
        )
        ...
        self.log_tb = customtkinter.CTkTextbox(
            self, height=80, font=customtkinter.CTkFont(size=11),
            fg_color=BG_DARK, border_color=BORDER, border_width=1, state="disabled",
        )
        ...
        self.btn_start = customtkinter.CTkButton(
            btn_frame, text="▶ Iniciar fila", command=self._start_queue,
            fg_color=ACCENT_GREEN, hover_color="#059669",
        )
        self.btn_start.grid(row=0, column=0, padx=4)

        self.btn_cancel = customtkinter.CTkButton(
            btn_frame, text="⏹ Cancelar", command=self._cancel_queue,
            fg_color=ACCENT_RED, state="disabled",
        )
    ...
    def _worker_queue(self, names: list[str], franchise: str):
        ...
        for i, char in enumerate(names):
            if self._cancel.is_set():
                ...
                return
            self.after(0, lambda c=char, idx=i: self.status_lbl.configure(
                text=f"Processando {idx+1}/{total}: {c}", text_color=ACCENT
            ))
            try:
    ...
    def _finish_queue(self, final_msg: str):
        self._running = False
...
class EditorScreen(customtkinter.CTkFrame):
    ...
    def _build_toolbar(self):
        ...
        customtkinter.CTkButton(
            bar, text="← Projetos", command=self.on_close,
            width=100, height=30, fg_color="transparent",
            border_width=1, border_color=BORDER, hover_color=BG_HOVER,
            font=customtkinter.CTkFont(size=12),
        ).grid(row=0, column=0, padx=(12, 8), pady=9)

        customtkinter.CTkLabel(
            bar, text=f"⚒  {self.project.name}",
            font=customtkinter.CTkFont(size=15, weight="bold"), text_color=TEXT,
        ...
        customtkinter.CTkButton(
            actions, text="📜 Akáshico", command=self._import_akashic,
            width=100, height=28, fg_color=BG_CARD, hover_color=BG_HOVER,
            font=customtkinter.CTkFont(size=12),
        ).pack(side="left", padx=3)

        customtkinter.CTkButton(
            actions, text="🌐 Wiki", command=lambda: WikiImportDialog(self, self.project),
            width=80, height=28, fg_color=BG_CARD, hover_color=BG_HOVER,
            font=customtkinter.CTkFont(size=12),
        ).pack(side="left", padx=3)

        self.btn_start = customtkinter.CTkButton(
            actions, text="▶  Run", command=self.start_pipeline,
            width=90, height=28, fg_color=ACCENT_GREEN, hover_color="#059669",
            font=customtkinter.CTkFont(size=12, weight="bold"),
        )
        self.btn_start.pack(side="left", padx=3)

        self.btn_cancel = customtkinter.CTkButton(
            actions, text="⏹", command=self.cancel_pipeline,
            width=36, height=28, fg_color=ACCENT_RED, state="disabled",
        )
    ...
    def _build_sidebar(self):
        ...
        customtkinter.CTkLabel(
            sec, text="MODELOS", font=customtkinter.CTkFont(size=10, weight="bold"),
            text_color=TEXT_DIM,
        ).grid(row=0, column=0, columnspan=2, padx=10, pady=(8, 4), sticky="w")

        for i, (lbl, attr, val) in enumerate([
            ("Fase 1", "model_draft_entry", MODEL_DRAFTING),
            ("Fase 2", "model_refine_entry", MODEL_REFINING),
            ("Fase 3", "model_summ_entry", MODEL_SUMMARIZING),
        ], start=1):
            customtkinter.CTkLabel(sec, text=lbl, font=customtkinter.CTkFont(size=11), text_color=TEXT_DIM).grid(
                row=i, column=0, padx=(10, 4), pady=2, sticky="w"
            )
            ent = customtkinter.CTkEntry(
                sec, height=26, font=customtkinter.CTkFont(size=11),
        ...
        customtkinter.CTkLabel(sec, text="").grid(row=4, column=0)

        has_ak = self.project.akashic_model_path.exists() or self.project.akashic_path.exists()
        self.akashic_lbl = customtkinter.CTkLabel(
            side,
            text="● Registro Akáshico OK" if has_ak else "○ Sem Registro Akáshico",
            text_color=ACCENT_GREEN if has_ak else ACCENT_RED,
        ...
        customtkinter.CTkLabel(
            hdr, text="CAPÍTULOS", font=customtkinter.CTkFont(size=10, weight="bold"),
            text_color=TEXT_DIM,
        ).grid(row=0, column=0, sticky="w")

        customtkinter.CTkButton(
            hdr, text="+", width=28, height=24, command=self._add,
            fg_color=ACCENT_GREEN, hover_color="#059669", font=customtkinter.CTkFont(size=14, weight="bold"),
        ).grid(row=0, column=1, padx=(4, 0))

        customtkinter.CTkButton(
            hdr, text="−", width=28, height=24, command=self._rem,
            fg_color=BG_CARD, hover_color=ACCENT_RED, font=customtkinter.CTkFont(size=14, weight="bold"),
        ).grid(row=0, column=2, padx=(2, 0))

        self.queue_scroll = customtkinter.CTkScrollableFrame(
            side, fg_color=BG_SIDE, scrollbar_button_color=BORDER,
        )
    ...
    def _build_main(self):
        ...
        self.tabview = customtkinter.CTkTabview(
            main, fg_color=BG_PANEL, segmented_button_fg_color=BG_SIDE,
            segmented_button_selected_color=ACCENT, segmented_button_selected_hover_color="#2563eb",
            segmented_button_unselected_color=BG_CARD, segmented_button_unselected_hover_color=BG_HOVER,
        ...
        self.draft_tb = customtkinter.CTkTextbox(
            self.tabview.tab("Rascunho"), state="disabled", font=font, fg_color=BG_DARK,
        )
        self.draft_tb.pack(fill="both", expand=True, padx=2, pady=2)

        self.final_tb = customtkinter.CTkTextbox(
            self.tabview.tab("Capítulo Final"), state="disabled", font=font, fg_color=BG_DARK,
        )
        self.final_tb.pack(fill="both", expand=True, padx=2, pady=2)

        self.summary_tb = customtkinter.CTkTextbox(
            self.tabview.tab("Resumo"), state="disabled", font=font, fg_color=BG_DARK,
        )
        ...
        customtkinter.CTkButton(
            rf, text="💾 Salvar Roster", command=self._save_roster,
            fg_color=ACCENT_GREEN, height=28, width=140,
        ).pack(pady=6)
        ...
        customtkinter.CTkButton(
            tf, text="💾 Salvar Open Threads", command=self._save_threads,
            fg_color=ACCENT_GREEN, height=28, width=160,
        ).pack(pady=6)

        self.log_tb = customtkinter.CTkTextbox(
            self.tabview.tab("Log"), state="disabled", font=customtkinter.CTkFont(size=12),
            fg_color=BG_DARK, text_color=TEXT_DIM,
        )
    ...
    def _build_statusbar(self):
        ...
        self.status_label = customtkinter.CTkLabel(
            bar, text="Pronto.", anchor="w", font=customtkinter.CTkFont(size=11), text_color=TEXT_DIM,
        )
    ...
    def _import_akashic(self):
        ...
        if path:
            ...
            messagebox.showinfo("Sucesso", "Registro Akáshico importado.\nRode o build script para gerar a versão curta.")
    ...
    def _refresh_queue_list(self):
        ...
        for i, item in enumerate(self.queue_items):
            ...
            btn = customtkinter.CTkButton(
                self.queue_scroll,
                text=f"  {STATUS_ICONS.get(item['status'], '○')}  {item['title']}",
                anchor="w",
                ...
                height=30,
                font=customtkinter.CTkFont(size=12),
                command=lambda idx=i: self._select_queue_item(idx),
            )
...
class LauncherScreen(customtkinter.CTkFrame):
    def __init__(self, parent, on_open_project):
        ...
        customtkinter.CTkLabel(
            header, text="⚒  Forja de Ficção IDE",
            font=customtkinter.CTkFont(size=22, weight="bold"), text_color=TEXT,
        ).grid(row=0, column=0, padx=24, pady=16, sticky="w")

        customtkinter.CTkButton(
            header, text="＋  Novo Projeto", command=self._new_project,
            fg_color=ACCENT_GREEN, hover_color="#059669", width=140, height=34,
            font=customtkinter.CTkFont(size=13, weight="bold"),
        ).grid(row=0, column=1, padx=24, pady=14)

        self.list_frame = customtkinter.CTkScrollableFrame(
            self, fg_color=BG_DARK, label_text="",
    ...
    def _refresh_list(self):
        ...
        if not projects:
            customtkinter.CTkLabel(
                ...
                text_color=TEXT_DIM, font=customtkinter.CTkFont(size=14),
        ...
        for i, p in enumerate(projects):
            ...
            customtkinter.CTkLabel(
                card, text=p["name"],
                font=customtkinter.CTkFont(size=16, weight="bold"), text_color=TEXT,
            ...
            customtkinter.CTkLabel(
                card, text=meta, text_color=TEXT_DIM, font=customtkinter.CTkFont(size=11),
            ...
            customtkinter.CTkButton(
                actions, text="Abrir", command=lambda path=p["path"]: self._open(path),
                fg_color=ACCENT, hover_color="#2563eb", width=100, height=32,
            ).pack(side="left", padx=4)

            customtkinter.CTkButton(
                actions, text="Excluir", command=lambda path=p["path"], name=p["name"]: self._delete(path, name),
                fg_color=BG_HOVER, hover_color=ACCENT_RED, width=80, height=32,
            ).pack(side="left", padx=4)
